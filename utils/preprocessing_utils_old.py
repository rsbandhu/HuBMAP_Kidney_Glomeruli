...
def split_image_into_tiles(img, mask, reduce=4, sz=256):
    ...
    print("shape final tile: {}, shape final mask: {}, number of tiles and mask = {}, {}".format( img_reshaped.shape,
          mask_tiled.shape, img_reshaped.shape[0], mask_tiled.shape[0]))
...
def save_thresholded_image(img, mask, raw_img_name, sz, output_dir, mask_tile_dict, sat_threshold=40, pixcount_th=200):
    ...
    pixcount_th = 200
    valid_idx = []
    print("Original tiled image count = {}".format(n))
    ...
    print("Image count after thresholding = {}".format(valid_img_count))

def image_transform():

    '''
    :return: a list of transformations from albumentations library
    '''

    transforms_list = []

    blur = a.Blur( p=1, blur_limit=3 )
    transforms_list.append(blur)

    ## Horizontal Flip
    h_flip = a.HorizontalFlip( p=1 )
    transforms_list.append(h_flip)

    v_flip = a.VerticalFlip( p=1 )
    transforms_list.append(v_flip)

    transpose = a.Transpose( p=1 )
    transforms_list.append(transpose)

    ran_gamma = a.RandomGamma( p=1 )
    transforms_list.append(ran_gamma)

    rand_bright = a.RandomBrightness( p=1 )
    transforms_list.append(rand_bright)

    rand_contrast = a.RandomContrast( p=1 )
    transforms_list.append(rand_contrast)

    g_noise = a.GaussNoise( p=1 )
    transforms_list.append(g_noise)

    rotation = a.Rotate(p=1 )
    transforms_list.append(rotation)
